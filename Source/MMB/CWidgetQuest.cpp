...
#include "CWidgetQuest.h"

void UCWidgetQuest::NativeOnListItemObjectSet(UObject* ListItemObject)
{
	...
	if (UCQuestData* QD = Cast<UCQuestData>(QuestData))
	{
		...
		if (ACPlayerController* PCC = Cast<ACPlayerController>(GetOwningPlayer())) RefreshQuestRecap(PCC);
	}
}

void UCWidgetQuest::RefreshQuestRecap(ACPlayerController* PCC, UClass* AchievedClass)
{
